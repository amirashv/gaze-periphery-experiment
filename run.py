...
-------------
  Detect  →  Calibration  →  Validate
    →  Intro  →  Quick  →  Fixation  →  Stimulus  →  Response  →  ITI
...
- 2-up/1-down adaptive staircase starting at magnitude 20°
- Bayesian Beta-Binomial abandon (P < 0.80 after min 10 trials)
- Break every 200 trials: 5-min countdown, then re-calibration
- R key = manual recalibration at any time
- Fixation monitoring with 5° radius
- Stimuli preloaded at startup
- Keyboard RT and SRT recorded per trial
...
def main():
    ...
    ecc_range = (5.0, min(30.0, max_ecc))
    ...
    def prepare_trial():
        ...
        print(f"Trial {total_trial_count+1}: cond={current_condition}, "
